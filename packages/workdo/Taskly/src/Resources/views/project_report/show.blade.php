                                    ...
                                    <tbody>
                                        @foreach ($project->users as $user)
                                            @php
                                                ...
                                                $total_user_task = Workdo\Taskly\Entities\Task::where('project_id', $project->id)
                                                    ->whereRaw('FIND_IN_SET(?,  assign_to) > 0', [$user->id])
                                                    ->get()
                                                    ->count();

                                                $all_task = Workdo\Taskly\Entities\Task::where('project_id', $project->id)
                                                    ->whereRaw('FIND_IN_SET(?,  assign_to) > 0', [$user->id])
                                                    ->get();

                                                $total_complete_task = Workdo\Taskly\Entities\Task::join('stages', 'stages.id', '=', 'tasks.status')
                                            ...
                                            <div class="col-xl-9">
                                                <div class="row justify-content-end align-items-center">
                                                    ...
                                                    <div class="col-auto mt-4">
                                                        ...
                                                        <a href="{{ route('project_report.show',$project->id) }}" class="btn btn-sm btn-danger" data-bs-toggle="tooltip" title="" data-original-title="{{ __('Reset') }}">
                                                            <span class="btn-inner--icon"><i class="ti ti-trash-off text-white-off "></i></span>
                                                        </a>
