...
<script>
    ...
    $(window).on('load', function () {
        ...
        // Prepare footer text (plain text) from settings to draw on every page
        var footerText = {!! json_encode(strip_tags($settings['proposal_footer_text'] ?? '')) !!};
        ...
        worker.toPdf().get('pdf').then(function (pdf) {
            ...
            for (var i = 1; i <= totalPages; i++) {
                ...
                if (footerLines.length > 0) {
                    ...
                    var startY = barTopY + paddingY + lineHeight; // draw downward from inside bar
