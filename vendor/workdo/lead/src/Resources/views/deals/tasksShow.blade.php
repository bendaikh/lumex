    ...
    <div class="row text-sm">
        <div class="col-md-4">
            <div class="mb-4">
                <b>{{__('Status')}}</b>
                @if($task->status)
                    <div class="badge rounded p-2 px-3 bg-success mb-1">{{__(Workdo\Lead\Entities\DealTask::$status[$task->status])}}</div>
                @else
                    <div class="badge rounded p-2 px-3 bg-warning mb-1">{{__(Workdo\Lead\Entities\DealTask::$status[$task->status])}}</div>
                @endif
            </div>
        </div>
        <div class="col-md-4">
